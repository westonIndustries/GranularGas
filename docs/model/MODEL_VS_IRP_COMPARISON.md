# Bottom-Up Model vs NW Natural IRP Forecast

## Overview

This document compares the bottom-up end-use model against NW Natural's official IRP (Integrated Resource Plan) top-down forecast. The two approaches use fundamentally different methodologies to project residential natural gas demand, and the differences in their outputs reveal important insights about the drivers of demand decline.
...
- Applies a fixed annual decay rate of **−1.19%/year**
- Decay rate is derived from historical UPC trends (2005–2025)
- No visibility into what drives the decline — it is a statistical trend extrapolation
...
```
therms(i, y) = (HDD × heating_factor × vintage_mult × segment_mult × qty) / efficiency(i, y)
```

Year-over-year changes are driven by:
...
- **SF/MF segment shift** from Census B25024 historical trend

The model currently simulates **space heating only**. Non-heating end-uses (water heating, cooking, drying, fireplaces) are estimated using RECS Pacific division proportions when `use_recs_ratios=true`.

---

## Results Comparison

### Space Heating Only (Model's Direct Output)
...
### Estimated Total UPC (Model + RECS Non-Heating Ratios)

When `use_recs_ratios=true`, non-heating end-uses are added using RECS Pacific division proportions:
...
## Why the Model is Higher Than the IRP

### 1. RECS Non-Heating Estimates Are Inflated

The model estimates non-heating end-uses using RECS 2020 Pacific division ratios. The Pacific division includes California, which has:
...
The heating factor was calibrated against 2025 billing data, which only covers January–March (the coldest quarter). A full-year calibration against 2024 data would be more robust and may reduce the baseline UPC.
...
## Why the Model Declines Faster Than the IRP
...
The model's value is not in matching the IRP exactly — it is in providing visibility into the **drivers** of demand decline:
...
- **[FORMULAS.md](FORMULAS.md)** — IRP formula and UPC calculation
- **[ALGORITHM.md](ALGORITHM.md)** — Full simulation algorithm
- **[EFFICIENCY_MODEL.md](EFFICIENCY_MODEL.md)** — Equipment efficiency mechanisms driving demand decline
